...
def _resolve_block_class(block_class_name):
    """Resolve a block class from src.core.blocks (host) or src.psm.blocks."""
    block_class = getattr(blocks, block_class_name, None)
    if block_class is not None:
        return block_class
    if block_class_name in ("EventfulPSMBlock", "EventfulPSMTokenwiseBlock"):
        import src.psm.blocks as psm_blocks
        return getattr(psm_blocks, block_class_name)
...
class ViTBackbone(ExtendedModule):
    ...
    def __init__(
        ...
        for i in range(depth):
            ...
            self.blocks.append(
                _resolve_block_class(block_class_i)(input_size=input_size, **block_config_i)
            )
