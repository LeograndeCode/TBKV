#!/usr/bin/env python3
"""
Verify every reported ViTDet-B / ImageNet VID (672) number in the TempoMem
...
the command in run_paper_results.sh.
...
TABLE = [
    ...
    ("TempoMem (layered)",
     "tbkv_eventful_filter_672-token_top_k=[512]-cache_reuse=0.25-"
...
def check(label, d, kblk, exp, n_exp, asserts):
    ...
    status = "ok  " if (ok_m and ok_g and ok_c) else "FAIL"
    if not (ok_m and ok_g and ok_c):
...
for cr, mi, exp in ABL:
    d = RES / (f"tbkv_eventful_filter_672-token_top_k=[512]-warmup=4-"
