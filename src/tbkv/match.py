...
def perform_tbkv_matching(
    ...
    if split_tokens:
        x_bg, x_fg, idx_bg, idx_fg = extract_bg_fg_tokens(x, old_attn, bg_ratio)  # [B, N_bg, C], [B, N_fg, C]
    else:
        x_bg = x
        x_fg = x[:, :0, :]

    # Apply matching algorithm on background tokens only
    k_matched, v_matched, matched_cache_tokens, matched_cache_norm, idx_matched, idx_unmatched = cache.match_tokens(
    ...
    # Uses the post-LayerNorm cache view, since unm_bg_tokens/x_fg are normed.
    tokens_forward = torch.cat([matched_cache_norm, unm_bg_tokens, x_fg], dim=-2)

    # For residual connection, we need unnormalized version
    if x_unnorm is not None:
        if split_tokens:
            x_bg_unnorm, x_fg_unnorm, _, _ = extract_bg_fg_tokens(x_unnorm, old_attn, bg_ratio)
        else:
...
def perform_tbkv_matching_with_tome(
    ...
    if r > 0:
        ...
        if x_unnorm is not None:
            if split_tokens:
                _, x_fg_unnorm, _, _ = extract_bg_fg_tokens(x_unnorm, old_attn, bg_ratio)
            else:
                x_fg_unnorm = x_unnorm[:, :0, :]
    ...
    # Uses the post-LayerNorm cache view, since unm_bg_tokens/x_fg are normed.
    tokens_forward = torch.cat([matched_cache_norm, unm_bg_tokens, x_fg_reduced], dim=-2)
    ...
    if x_unnorm is not None:
        if split_tokens:
            x_bg_unnorm, _, _, _ = extract_bg_fg_tokens(x_unnorm, old_attn, bg_ratio)
        else:
            x_bg_unnorm = x_unnorm
        unm_bg_tokens_unnorm = gather(x_bg_unnorm, dim=-2, index=idx_unmatched.unsqueeze(-1).expand(-1, -1, C))
