"""Shared TBKV filter: the one idea reused across base methods.

Every "<method> + TBKV" integration reduces to the same operation: the base
method (Eventful, MaskVD, ...) proposes a set of tokens to compute this frame;
TBKV drops the fraction of them that best match a content cache, because a token
...
def tbkv_keep_mask(cand_feat, cache_feat, cache_reuse, protect=None):
