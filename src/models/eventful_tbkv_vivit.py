"""ViViT with EventfulTBKV spatial blocks.

The only thing this adds over the stock FactorizedViViT is caching/matching-mode
control, so the evaluation harness can run the same two-pass protocol as TBKV:
...
subclass is needed -- FactorizedViViT builds EventfulTBKVBlock directly.
"""

import torch

from src.models.vivit import FactorizedViViT
from src.eventful_tbkv.blocks import EventfulTBKVBlock


class EventfulTBKVViViT(FactorizedViViT):
    ...
    def _blocks(self):
        for module in self.modules():
            if isinstance(module, EventfulTBKVBlock):
                yield module
