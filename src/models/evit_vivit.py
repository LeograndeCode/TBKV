...
from src.evit.backbones import EVITBackbone as EViTBackbone
from src.evit.blocks import EVITBlock, LN_EPS
from src.core.base import ExtendedModule
from src.core.counting import CountedLinear


from src.utils import as_float32, resize_to_fit
...
# EVIT Edit:
# We just added the set_mode() method to switch between caching and matching modes, and
# made some minor adjustments to the forward() method to accommodate this.
#  The caching/matching mode and cache state are still managed at the block level (i.e., in EVIT Block), 
# but the set_mode() method allows us to switch modes at the model level, which is 
# more convenient for evaluation loops. We also added a set_caching_mode() method to set 
# the caching mode of the backbone, which is necessary for EVIT since the backbone 
# is responsible for caching key/value pairs during the caching pass.


class EVITFactorizedViViT(ExtendedModule):
    """
    ...
    def __init__(
        ...
        self.set_mode("caching")

    def set_mode(self, mode):
        """Switch all EVITBlocks between 'caching' and 'matching' modes."""
        caching = (mode == "caching")
        for module in self.modules():
            if isinstance(module, EVITBlock):
    ...
    def _forward_view(self, x):
        ...
        # NOTE: Do NOT call self.spatial_model.reset() here for EVIT.
        # The cache state (populated during the caching pass) and the
        # caching/matching mode (set via set_mode()) must be preserved
        # across the caching → matching pass boundary.
        # The outer model.reset() in the evaluation loop handles full
        # resets between videos.
        x = torch.stack([self.spatial_model(x[:, t]) for t in range(x.shape[1])], dim=1)
        # (batch, time, dim)

        return x

    def set_caching_mode(self, caching):
        self.spatial_model.backbone.set_caching_mode(caching)
        #self.temporal_model.backbone.set_caching_mode(caching)
...
def main():
    ...
    model = EVITFactorizedViViT(
        ...
        spatial_config={
            ...
            "depth": 1,
            "position_encoding_size": (8, 8),
            "block_class": "EVITBlock",
        },
        spatial_views=1,
        temporal_config={
            "block_config": {
    ...
    model.eval()
    x = torch.randn(2, 4, 3, 32, 32, device=device)
    with torch.no_grad():
        out = model(x)

    print("input shape:", tuple(x.shape))
    print("output shape:", tuple(out.shape))
    print("output sample:", out[0].detach().cpu())

    model.set_mode("matching")  # Make sure we're in matching mode for this test.
    x = torch.randn(2, 4, 3, 32, 32, device=device)
    with torch.no_grad():
        out = model(x)
