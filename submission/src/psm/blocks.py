"""EventfulPSM: the REAL Eventful block, with PSM as a recompute-set filter.

The PSM logic lives in a mixin so it can sit on top of BOTH Eventful block
variants unchanged:

  * EventfulPSMBlock          <- EventfulBlock          (global-attention / ViViT)
  * EventfulPSMTokenwiseBlock <- EventfulTokenwiseBlock (windowed / ViTDet)

With PSM disabled (cache_reuse=0) each is bit-for-bit its base Eventful block.
PSM then only ever *subtracts* work: Eventful's qkv gate proposes the tokens to
recompute this frame, and PSM drops the fraction that best match a content cache
...
The content-matching policy itself is the shared psm_keep_mask (src/psm/filter),
the same one MaskVD+PSM uses.
"""
...
class _EventfulPSMMixin:
    """Adds the PSM recompute-set filter to an Eventful block. Mix in FIRST."""
    ...
    def _compute_forced_index(self, gin):
        ...
        with torch.no_grad():
            ...
            keep = psm_keep_mask(feat, self.cache.tok, self.cache_reuse, protect=protect)
            self.dbg_proposed += int(B * k)
            self.dbg_recomputed += int(keep.sum())
            # psm_keep_mask keeps a fixed count per row, so the kept set is
    ...
    def _prepare_substitution(self, cand, feat, keep, n_keep):
        ...
        # Best-matching cache token per candidate (same metric as psm_keep_mask).
...
class EventfulPSMBlock(_EventfulPSMMixin, EventfulBlock):
    """Global-attention Eventful + PSM (ViViT spatial, ViTDet global layers)."""


class EventfulPSMTokenwiseBlock(_EventfulPSMMixin, EventfulTokenwiseBlock):
    """Windowed Eventful + PSM (ViTDet windowed layers)."""
