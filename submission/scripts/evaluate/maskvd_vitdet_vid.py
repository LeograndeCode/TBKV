...
Runs from PSM root. Imports MaskVD codebase from MaskVD/.
...
PSM_ROOT = Path(__file__).resolve().parents[2]
MASKVD_ROOT = PSM_ROOT / "MaskVD"
if not MASKVD_ROOT.is_dir():
    sys.exit(
        "MaskVD baseline: this row needs the third-party MaskVD implementation,\n"
        "which is cited in the paper but not redistributed in this archive.\n"
        f"Place (or symlink) that repository at {MASKVD_ROOT} and re-run.\n"
        "Every other row of Table 1 runs with no external code."
    )
...
def run_evaluation(device, model, data, n_items, period=4, conf=0.5, margin=0, frame_stride=1, warmup=0,
    ...
    # ── Latency / peak-memory harness (identical protocol to PSM script) ─────
...
def main():
    ...
    weights_path = PSM_ROOT / "weights" / "vitdet_b_vid.pth"
    data_root = PSM_ROOT / "data" / "vid"
    detectron2_config = str(PSM_ROOT / "configs" / "detectron" / "vitdet_b_vid.py")
