#!/usr/bin/env python3
"""EventfulPSM (real Eventful + recompute-set filter) on ImageNet-VID / ViTDet.

Same as the stock Eventful ViTDet eval, but two-phase per video: the first
`warmup` frames run in caching mode (Eventful builds its state AND PSM builds
its content cache -- these frames are NOT scored or counted), then the remaining
frames run in matching mode, where PSM drops cache-matched tokens from
...
def evaluate_eventful_psm_vitdet(device, model, data, config):
    ...
    #   the same warm-up harness as scripts/evaluate/vitdet_vid.py.
...
def main():
    ...
    # Route PSM overrides into the block config.
    ...
    if cr_sweep is None:
        run_evaluations(config, ViTDet, data, evaluate_eventful_psm_vitdet)
        return
    ...
    for cr in cr_sweep:
        ...
        run_evaluations(run_config, ViTDet, data, evaluate_eventful_psm_vitdet)
