...
def evaluate_eventful_tbkv(device, model, data, config):
    ...
    if replay_matching:
        return (
            "EventfulTBKV — ViViT / Kinetics-400 (replay_matching)\n"
...
def run_ablation_sweep(device, model, data, config, cache_reuse_values, merge_iterations_values):
    ...
    attributes on each EventfulTBKVBlock, so no rebuild/reload is needed
    between combos -- see src/eventful_tbkv/blocks.py). Writes every combo's
...
def main():
    ...
    if sweep:
        ...
        if spatial_cfg is not None:
            ...
            for key in ("token_keep", "merge_ratio", "tbkv_tau", "caching", "substitute"):
                if key in config:
                    spatial_cfg[key] = config[key]

        device = config.get("device", get_pytorch_device())
        model = EventfulTBKVViViT(**config["model"])
