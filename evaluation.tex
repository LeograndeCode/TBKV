% =========================================================================
% Evaluation - TempoMem (revised after AAAI-style self-review)
...
the method in opposite ways, and we report both the positive and the
negative findings: content-based reuse is highly effective for class-token
readouts and is currently dominated by per-position gating on dense spatial
prediction. The section ``When Does Content-Based Reuse Help?'' analyzes
why.
...
described in the Methodology. For backbone-scaling experiments we
additionally use VideoMAE \cite{videomae} with ViT-S/B/L/H encoders on
Kinetics-400 and UCF101 \cite{ucf101}. All weights are frozen, pretrained
...
\emph{MaskVD} \cite{maskvd} skips background tokens using detection masks.
\emph{ToMe} \cite{tome} is a single-frame token-merging method that we also
compose with TempoMem on foreground tokens.

\paragraph{Metrics.}
Kinetics-400 and UCF101 report Top-1 and Top-5 accuracy; ImageNet VID
reports mAP@50. Compute is reported as GFLOPs, measured with in-code
...
into
real speedups (56 to 58\,ms vs.\ 69.6\,ms dense). Standalone TempoMem does
not yet: matching and cache management add overhead that outweighs the
saved projections in the current implementation (101.8\,ms), although its
memory footprint (929\,MB) stays close to dense and far below Eventful's
buffers (2118\,MB), reflecting the merged cache of
...
the
finest prototypes the memory budget allows. This is direct evidence that
classification tolerates coarse summaries while detection consumes the
per-token detail that merging removes.
